...
class SensorsProcessingService:
    ...
    def process_sensor_data(self) -> Dict[int, ProcessedSensorData]:
        ...
        for sensor_raw_data in sensors_raw_data:
            ...
            processed_data = sensor.get_processed_data()
            processed_sensors_data[processed_data.id] = processed_data
        return processed_sensors_data

    def get_calculated_data(
            self, processed_sensors_data: Dict[int, ProcessedSensorData]
    ) -> Dict[int, CalculatedData]:
        ...
        for raw_data in raw_values_to_calculate:
            ...
            calculated_data[raw_data.settings.id] = calculated.get_calculated_value()
        return calculated_data

    def _get_raw_data_to_calculate(
            self,
            values_to_calculate: List[SettingsCalculate],
            processed_sensors_data: Dict[int, ProcessedSensorData]
